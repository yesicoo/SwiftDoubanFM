...
extension UIImage {
    ...
    func CropToScreen(let screenWidth:Int,let screenHeight:Int)->UIImage{
        let imgWidth:Int=Int(self.size.width);
        let imgHeight:Int=Int(self.size.height);
        var p_x:Int=0;
        var p_y:Int=0;
        var cut_Width:Int=0;
        var cut_Height:Int=0;
            if Float(imgWidth)/Float(screenHeight)>=Float(imgWidth)/Float(screenWidth){
                cut_Height=Int(Float(imgWidth*screenHeight)/Float(screenWidth));
                cut_Width=imgWidth;
                p_x=0;
                p_y=(imgHeight-cut_Height)/2;
                return crop(CGRect(x: p_x,y: p_y,width: cut_Width,height: cut_Height))!;
            }else{
                cut_Width=Int(Float(imgHeight*screenWidth)/Float(screenHeight));
                cut_Height=imgHeight;
                p_x=(imgWidth-cut_Width)/2;
                p_y=0;
                return crop(CGRect(x: p_x,y: p_y,width: cut_Width,height: cut_Height))!;
            }
    }
